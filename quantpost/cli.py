"""Command line: `quantpost <command>`.

    quantpost doctor                     # what works, what needs a key
    quantpost sources                    # list adapters and curated series
    quantpost fetch fred ust_10y vix --start 2015-01-01 --out data.csv
    quantpost run exp001_lorenz_esn      # run an experiment -> post + figures
    quantpost audit build/*.manifest.json
    quantpost publish exp001_lorenz_esn --medium
"""
...
def cmd_fetch(args) -> int:
    mod = importlib.import_module(f".sources.{args.source}", package="quantpost")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="quantpost",
