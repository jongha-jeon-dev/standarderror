"""Public-data adapters.
    ...
    from standarderror.sources import fred, ecb, ecos, bis
    df = fred.get(["ust_10y", "ust_2y", "vix"], start="2015-01-01")

Every adapter returns a DatetimeIndex-ed frame with provenance in
`df.attrs["standarderror"]`. `citations(df)` turns that into the footer line a post
...
def _blocks(df: pd.DataFrame) -> list[dict]:
    if "standarderror_sources" in df.attrs:
        return list(df.attrs["standarderror_sources"])
    one = df.attrs.get("standarderror")
