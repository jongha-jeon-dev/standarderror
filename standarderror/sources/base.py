...
named `date`, with one column per series, plus a `.attrs["standarderror"]` dict
...
def tidy(
    ...
    *,
    ...
    out.attrs["standarderror"] = payload
    return out
...
def merge_sources(*frames: pd.DataFrame) -> pd.DataFrame:
    ...
    sources = []
    for f in frames:
        p = f.attrs.get("standarderror")
        if p and p not in sources:
            sources.append(p)
    out.attrs["standarderror_sources"] = sources
    return out
