...
### `schoolmath/` — our own model weights, and no corpus at all

`schoolmath.pt` is a checkpoint of an 804,096-parameter character-level
transformer trained by `scripts/train_schoolmath.py`. Same architecture as
`tiny_gpt` — four blocks, four heads, width 128, context 64 — on an
eighteen-character vocabulary instead of sixty-five, which is where the
12,032-parameter difference comes from.

sha256 `c40b480c51c4c6fb50e94d223ba636c76914fa3c9c2815f7d925223925afe819`,
verified by `standarderror.schoolmath.model.load`.

There is no corpus to license or to withhold. The training text is arithmetic,
generated by `standarderror.schoolmath.curriculum` and deterministic in its
seed, so a reader rebuilds the exact training stream from the source rather
than downloading it. No external data of any kind went into this model.
